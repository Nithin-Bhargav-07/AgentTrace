...
export function validateTraceBytes(
  ...
  if (
    ...
    rawDocument.schemaVersion === "agent-trace.native-trace.v1"
  ) {
...
export function validateAuthorityDraft(
  ...
  const candidate = {
    schemaVersion: "agent-trace.authority.v1",
  ...
  for (const incident of incidents) {
    ...
    if (ruleIds.has("AR-DATA-001") || ruleIds.has("AR-EGRESS-001")) {
      ...
      addAction(incident, "contain", {
        ...
        description: includesSend
          ? `Use ${destination} delivery logs to identify the actual recipient scope. If policy and platform controls permit, an authorized responder can apply recall, access, or follow-up procedures; AgentTrace does not execute them.`
          : `Review sharing, retention, and access in ${destination}. If the exposure is confirmed, an authorized responder can revoke access or remove the copy after preserving required evidence; AgentTrace does not execute that action.`,
    ...
    if (
      ...
      addAction(incident, "prevent-repeat", {
        ...
        description: "Keep the recorded receipt unchanged. Update the agent or workflow so it cannot repeat the cited system, operation, or approval deviation, then run a new trace through AgentTrace. Do not retroactively widen the authority envelope to make this run appear clean.",
