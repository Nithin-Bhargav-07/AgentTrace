...
export const EVIDENCE_PACKET_SCHEMA_VERSION =
  "agent-trace.evidence-packet.v1" as const;
export const DECISION_BRIEF_SCHEMA_VERSION =
  "agent-trace.decision-brief.v1" as const;
...
export async function verifyEvidencePacket(
  ...
  if (!parsed.success) {
    gates.push(fail(
      "packet_contract",
      "The file is not a valid AgentTrace Evidence Packet or its cross-artifact references do not hold.",
