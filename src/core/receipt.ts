...
export async function buildReceipt(
  ...
  if (!isRecord(rawDocument) && !Array.isArray(rawDocument)) {
    return failure(
      "unsupported_format",
      `This schema is not supported. AgentTrace accepts ${NATIVE_TRACE_SCHEMA_VERSION}, one documented OTLP/JSON resourceSpans shape, or a JSON record array with an explicit mapping.`,
      retainedSource,
    );
  }
