...
export function ReceiptReviewApp(props: {
  ...
  async function handleFile(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (
      ...
      setIntakeError({
        message: "Choose a UTF-8 .json file. AgentTrace does not accept JSONL, ZIP, YAML, or binary input.",
      });
      return;
    }
  ...
  function downloadReceipt() {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `agent-trace-${safeTraceId}.json`;
  ...
  async function downloadEvidencePacket() {
    ...
    try {
      ...
      const safeTraceId = result.receipt.run.traceId.replace(/[^a-z0-9_-]+/gi, "-");
      anchor.href = url;
      anchor.download = `agent-trace-evidence-${safeTraceId}.json`;
      document.body.append(anchor);
      anchor.click();
      anchor.remove();
      URL.revokeObjectURL(url);
  ...
  async function downloadRecoveryPlan() {
    ...
    try {
      ...
      const safeTraceId = result.receipt.run.traceId.replace(/[^a-z0-9_-]+/gi, "-");
      anchor.href = url;
      anchor.download = `agent-trace-recovery-${safeTraceId}.json`;
      document.body.append(anchor);
      anchor.click();
      anchor.remove();
      URL.revokeObjectURL(url);
  ...
  return (
    <>
      <header className="app-header">
        <a className="brand" href="#top" aria-label="AgentTrace home">
...
function IntakeStep({
  ...
  async function handleVerificationFile(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (
      ...
      setVerificationError("Choose an exported AgentTrace receipt or evidence packet .json file.");
        ...
        <div className="custom-input-grid">
          ...
          <div className="paste-field">
            ...
            <textarea
              ...
              placeholder={'{\n  "schemaVersion": "agent-trace.native-trace.v1"\n}' }
            />
                ...
                <div className="custom-input-grid verifier-input-grid">
                  ...
                  <div className="paste-field">
                    ...
                    <textarea
                      ...
                      placeholder={'{\n  "schemaVersion": "agent-trace.evidence-packet.v1"\n}'}
                    />
                    <div className="field-action-row">
  ...
  return (
    <section id="recovery" className="recovery-section" aria-labelledby="recovery-title">
      <div className="recovery-heading">
        ...
        <p>
          These are deterministic, evidence-linked proposals. AgentTrace has not accessed
