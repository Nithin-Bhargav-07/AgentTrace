...
The Portable Receipt Verifier checks an exported AgentTrace without sending the file to a server, Granite, or any other service. It is designed for the moment after a receipt changes hands: a manager, auditor, or hackathon judge can ask whether the JSON still agrees with the deterministic evidence inside it.
...
5. Validate the strict `agent-trace.receipt.v1` schema and cross-object references.
...
A pass means the exported receipt is internally self-consistent under the current AgentTrace schemas and deterministic rules. In particular, its accounting, coverage, policy result, and cited manager copy agree with the evidence stored in that export.
