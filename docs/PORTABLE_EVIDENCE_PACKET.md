...
Portable Evidence Packet v1 is the manager handoff artifact for AgentTrace. It packages the decision-ready view and the replayable evidence contract into one strict JSON file without including the original trace.

The schema version is `agent-trace.evidence-packet.v1`.
...
| Receipt | The complete validated AgentTrace v1 artifact, including authority, canonical evidence, findings, coverage, cited copy, integrity metadata, and reviewer disposition |
