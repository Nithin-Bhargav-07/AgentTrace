...
|---|---|---|---|
| Native expected run | AgentTrace Native Trace v1, 3 raw events | `within_declared_authority` | Confirms that activity inside the supplied authority envelope is not over-flagged. |
| Native overreaching run | AgentTrace Native Trace v1, 6 raw events | `material_deviations_found` | Seeds six policy-rule families and checks that each is detected. |
...
10. The generic case loads a non-native `activity_log` plus `agent-trace.generic-json-mapping.v1`, verifies all ten records map, and produces the same qualified clean result as its equivalent Native Trace example.
